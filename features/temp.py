...
def get_data_pti(session, offset, count, ticker):
...
def get_data_rti(session, offset, count, ticker):
    with session() as session:
...
def get_data_rti_d(session, offset, count, ticker):
    with session() as session:
...
def get_data_rmd_d(session, offset, count, ticker):
    with session() as session:        
...
def get_data_rmd_d_rti(session, offset, count, ticker):
    with session() as session:        
...
def get_data_rmd_d_rti_d(session, offset, count, ticker):
    with session() as session:        
        ### Combine market data and technical indicators:
        query = (
            select(MarketData, EquityIndicators, SupClassifierDataset)
