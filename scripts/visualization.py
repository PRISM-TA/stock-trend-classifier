...
def create_trend_visualization(db_session, 
                             ticker: str,
                             results: List[Dict],
    ...
    market_stmt = select(MarketData).where(
    ...
    ax0_volume.bar(market_df['date'], market_df['volume'], alpha=0.3, color='gray', label='Volume')
    ...
    for label in range(3):
        ...
        ax1.scatter(real_labels_df[mask]['start_date'], 
    ...
    ax2_conf = ax2.twinx()
    ax2_conf.plot(pred_df['date'], pred_df['confidence'], 'b--', alpha=0.5, label='Confidence')
    ...
    accuracy_str = f"{overall_accuracy*100:.2f}" if overall_accuracy is not None else "NA"
